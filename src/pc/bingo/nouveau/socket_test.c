...
void socket_test() {
  ...
  // send_addr.sin6_addr.s_addr = inet_addr("127.0.0.1");
  ...
  if (sendto(sock, req, strlen(req), 0, (struct sockaddr*)&send_addr, sizeof(send_addr)) < 0) {
  ...
  if (recvfrom(sock, res, sizeof(res), 0, (struct sockaddr*)&recv_addr, &recv_addr_structlen) < 0) {
    if (errno == 11) {
